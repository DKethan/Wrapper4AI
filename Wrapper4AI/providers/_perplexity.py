"""Perplexity AI provider via HTTP API."""
...
class PerplexityProvider(BaseProvider):
    """Perplexity AI (Sonar) models via HTTP API."""
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        key = resolve_api_key("perplexity", explicit_key=api_key)
        if not key:
            raise AuthenticationError(
                "Perplexity API key is required", provider="perplexity"
            )
        self._api_key = key
    ...
    def generate(self, messages: list[dict[str, str]]) -> str:
        ...
        if r.status_code == 429:
            raise RateLimitError(
                "Perplexity rate limit exceeded", provider="perplexity"
            )
        r.raise_for_status()
        data = r.json()
        choices = data.get("choices")
        if not choices or not choices[0].get("message", {}).get("content"):
            raise APIError("Empty response from Perplexity", status_code=r.status_code)
